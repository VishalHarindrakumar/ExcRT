...
FEET_TOO_WIDE_SOUND = mixer.Sound('./audio/feet_too_wide.mp3')
FEET_TOO_TIGHT_SOUND = mixer.Sound('./audio/feet_too_tight.mp3')
KNEE_TOO_WIDE_SOUND = mixer.Sound('./audio/knee_too_wide.mp3')
KNEE_TOO_TIGHT_SOUND = mixer.Sound('./audio/knee_too_tight.mp3')
...
class SquatDetection:
    ...
    def load_machine_learning_model(self) -> None:
        ...
        self.audio_state = {
            "feet_too_wide": False,
            "feet_too_tight": False,
            "knee_too_wide": False,
            "knee_too_tight": False
        }

    def stop_all_sounds(self):
        FEET_TOO_WIDE_SOUND.stop()
        FEET_TOO_TIGHT_SOUND.stop()
        KNEE_TOO_WIDE_SOUND.stop()
        KNEE_TOO_TIGHT_SOUND.stop()
        for key in self.audio_state:
            self.audio_state[key] = False

    def handle_audio_feedback(self, feet_placement, knee_placement):
        # Handle feet placement audio
        if feet_placement == "too wide":
            if not self.audio_state["feet_too_wide"]:
                self.stop_all_sounds()
                FEET_TOO_WIDE_SOUND.play(-1)
                self.audio_state["feet_too_wide"] = True
        elif feet_placement == "too tight":
            if not self.audio_state["feet_too_tight"]:
                self.stop_all_sounds()
                FEET_TOO_TIGHT_SOUND.play(-1)
                self.audio_state["feet_too_tight"] = True
        
        # Handle knee placement audio
        if knee_placement == "too wide":
            if not self.audio_state["knee_too_wide"]:
                self.stop_all_sounds()
                KNEE_TOO_WIDE_SOUND.play(-1)
                self.audio_state["knee_too_wide"] = True
        elif knee_placement == "too tight":
            if not self.audio_state["knee_too_tight"]:
                self.stop_all_sounds()
                KNEE_TOO_TIGHT_SOUND.play(-1)
                self.audio_state["knee_too_tight"] = True
        
        # If no errors, stop all sounds
        if feet_placement == "correct" and (knee_placement == "correct" or knee_placement == "unknown"):
            self.stop_all_sounds()
    ...
    def detect(self, mp_results, image, timestamp) -> None:
        ...
        try:
            ...
            self.handle_audio_feedback(feet_placement, knee_placement)
...
squat_detection = SquatDetection()
...
try:
    while cap.isOpened():
        ...
        if mp_results.pose_landmarks:
            ...
            squat_detection.detect(mp_results, image, current_time)
    ...
    # Clean up
    squat_detection.stop_all_sounds()
    cap.release()
